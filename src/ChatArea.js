...
const socket = io('https://whatsapp-backend-hdr6.onrender.com'); // Connect to the Socket.IO server
...
function ChatArea() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('https://whatsapp-backend-hdr6.onrender.com/users');
        if (!response.ok) {
...
//console.log(updatedData);
  ...
  const fetchMessages = async (recipient) => {
    ...
    try {
      const response = await fetch(`https://whatsapp-backend-hdr6.onrender.com/messg/messages/${chatId}`);
      if (!response.ok) {
