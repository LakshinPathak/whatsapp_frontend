...
const socket = io('https://whatsapp-backend-new.onrender.com'); // Connect to the Socket.IO server
...
function ChatArea() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('https://whatsapp-backend-new.onrender.com/users');
        if (!response.ok) {
  ...
  const fetchMessages = async (recipient) => {
    ...
    try {
      const response = await fetch(`https://whatsapp-backend-new.onrender.com/messg/messages/${chatId}`);
      if (!response.ok) {
